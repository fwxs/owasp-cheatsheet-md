...
### 1. Secure by Design
    * Opt for a secure design at the beginning of development, not as an
      afterthought.
    * Keep in mind security principles like least privilege, defense in depth,
      and separation of concerns.
    * Follow industry standards and best practices, such as:
          o National Institute of Standards and Technology (NIST)
          o Internet Engineering Task Force (IETF)
For more information, see the Secure_Product_Design_Cheat_Sheet.
### 2. Secure APIs
...
### 3. Principle of Least Privilege
    ...
    * This applies not only to device permissions granted by the user, but also
      to permissions granted to the app by backend services.
    * Avoid storing application files with overly permissive permissions.
    * Secure by default: applications should have the most secure settings by
      default.
### 4. Supply Chain
...
See the Vulnerable_Dependency_Management_Cheat_Sheet for recommendations on
managing third-party dependencies when vulnerabilities are discovered.
## Authentication & Authorization
...
Authentication_Cheat_Sheet and M1:_Insecure_Authentication/Authorization from
...
### 2. Credential Handling
    ...
    * Do not store user credentials on the device. Consider using secure,
      revocable access tokens.
### 3. Passwords and PIN Policy
    * Require password complexity.
    * Do not allow short PINs such as 4 digits.
    * Use platform specific secure storage mechanisms, such as Keychain (iOS)
      or Keystore (Android).
### 4. Biometric Authentication
...
### 1. Data Encryption
    ...
    * Leverage hardware-based security features when available (e.g., Secure
      Enclave on iOS, Strongbox on Android) for key storage and cryptographic
...
See the Logging_Cheat_Sheet for examples of data that should not be logged.
### 3. Use HTTPS
...
### 2. Use Secure Protocols
    ...
    * Do not override SSL certificate validation to allow self-signed or
      invalid certificates.
    * Avoid mixed version SSL sessions.
    * Encrypt data even if sent over SSL, in case of future SSL
      vulnerabilities.
...
### 3. Certificate Pinning
    * Consider certificate pinning. See the Pinning_Cheat_Sheet for pros and
...
### 2. User Notifications
    * Inform the user about security-related activities, such as logins from
      new devices.
### 3. Input Validation
    * Validate and sanitize user input. See the Input_Validation_Cheat_Sheet
...
## Application Integrity
    ...
    * Implement runtime anti-tampering controls:
          o Check for signs of debugging, hooking, or code injection.
          o Detect if the app is running in an emulator or rooted/jailbroken
            device.
          o Verify app signatures at runtime.
          o Apply appropriate responses to detected tampering (e.g., limiting
            functionality).
## Testing
### 1. Penetration Testing
    * Perform ethical hacking to identify vulnerabilities.
    * Example tests:
          o Cryptographic vulnerability assessment.
          o Attempt to execute backend server functionality anonymously by
            removing any session tokens from POST/GET requests.
### 2. Automated Tests
    * Leverage automated tests to ensure that security features are working as
      expected and that access controls are enforced.
### 3. Usability Testing
    * Ensure that security features do not harm usability, which could cause
      users to bypass security features.
## Post-Deployment
### 1. Incident Response
    * Have a clear incident response plan in place.
### 2. Updates
    * Plan for regular updates and patches. In the case of mobile apps, this is
      especially important due to the delay between when a patch is released
      and when users actually receive the updated version due to app store
      review processes and the time it takes for users to update their apps.
    * Use a mechanism to force users to update their app version when
      necessary.
### 3. Monitoring and Analytics
...
### Android
    ...
    * Avoid storing sensitive data in SharedPreferences. See the Android_docs
      on working with data securely for more details.
    * Disable backup mode to prevent sensitive data being stored in backups.
    * Use Android Keystore with hardware backing (TEE or StrongBox) to securely
      store cryptographic keys.
          o How: Generate keys with hardware backing by specifying
            .setIsStrongBoxBacked(true) in KeyGenParameterSpec.Builder when
            available (Android 9+).
          o Verify hardware-backed storage via KeyInfo.isInsideSecureHardware
            ().
          o Fall back to regular hardware-backed keystore if Strongbox isn't
            available.
          o Configure key usage restrictions with
            .setUserAuthenticationRequired(true) for sensitive operations.
          o See Hardware-backed_Keystore_documentation.
    * Implement Google's Play_Integrity_API for device and app integrity
      checks.
          o How: Obtain an integrity verdict from the device, validate server-
            side, and take action if integrity checks fail.
          o The SafetyNet Attestation API was fully turned down in January
            2025. All developers must migrate to Play Integrity API.
          o See Play_Integrity_API_documentation.
### iOS and iPadOS
#### Shortcuts Permissions
    ...
    * There are several scenarios in which a user can execute a Shortcut while
      the device is locked:
   1. If a Shortcut is added as a widget to Today View, it can be accessed and
      executed while the device is locked.
   ...
   3. If a Shortcut is assigned to the Control Center (on iOS/iPadOS 18+), it
      can be executed by pulling up the Control Center and pressing the
      Shortcut button while the device is locked.
   4. A Shortcut can be invoked via Siri while the device is locked.
   5. If a Shortcut is added to the user's Home Screen (on iOS/iPadOS 18+), it
      can be directly executed by tapping the Shortcut button on the user's
      lock screen while the device is locked.
   6. If a Shortcut is set to run at a specific interval or a specific time, it
      can execute even if the device is locked.
    ...
    * How: Store secure tokens in Keychain that the app validates before
      ...
      UIApplication.shared.isProtectedDataAvailable to restrict execution of
      sensitive actions when the device is locked.
#### Siri Permissions
    * Siri can access app functionalities through voice or Type_to_Siri
    ...
    * How: Configure requiresUserAuthentication to true on intents that expose
      ...
      INIntent.userConfirmationRequired = true for operations requiring
      explicit user confirmation. These settings ensure proper authentication
      (e.g., Face ID or PIN) and explicit approval before Siri can execute
      sensitive commands. (For more information, see Apple Developer's SiriKit
      documentation.)
#### Deep Link Security
    ...
    * An example of this on Microsoft Authenticator for iOS (which was
      ...
      navigating to msauth://microsoft.aad.brokerplugin/?, which would open
      Authenticator and dismiss the Face ID/Touch ID/passcode prompt.
    * How: Implement authentication checks on any view controllers or endpoints
      accessed via deep links. Configure and validate Universal Links using
      apple-app-site-association files for secure deep linking. Sanitize and
      validate all parameters received through deep links to prevent injection
      attacks. Ensure unauthorized users are redirected to the login screen,
      preventing direct access to sensitive parts of the app without proper
      authentication. (See Apple Developer's Supporting_universal_links_in_your
...
#### WidgetKit Security
    ...
    * How: For iOS/iPadOS versions 17 and higher, use WidgetInfo.isLocked to
      detect lock screen state. For earlier iOS versions, implement custom
      ...
      aren't met. (See Apple's WidgetKit_security for more information.)
#### Additional Security Considerations
    * Configure appropriate background refresh policies to prevent sensitive
      data updates while the device is locked.
    ...
    * Use Apple's Secure Enclave for secure cryptographic key storage and
      sensitive operations.
          o How: Create keys using SecKeyCreateRandomKey with kSecAttrTokenID
            set to kSecAttrTokenIDSecureEnclave.
          o Keys created in the Secure Enclave never leave the secure hardware
            - only the operations using those keys are performed there.
          o For biometric operations, use LAContext with evaluatePolicy to
            perform authentication directly through the Secure Enclave without
            exposing biometric data to your application.
          o Consider access control options like kSecAccessControlBiometryAny
            or kSecAccessControlUserPresence to require user authentication
            before key usage.
          o See Secure_Enclave_documentation.
    * Use Apple's App_Attest_API (iOS 14+) to validate app integrity.
          o How: Generate attestation keys and assertions with
            DCAppAttestService and verify assertions server-side.
          o Complement with Apple's DeviceCheck_API for persistent device state
            tracking.
          o See App_Attest_documentation.
## Advanced Hardware Security & Monitoring
    ...
    * Consider additional runtime security measures (behavioral anomaly
      detection, runtime monitoring) to complement built-in OS protections in
      higher-risk scenarios.
For further reading, visit the OWASP_Mobile_Top_10_Project. For a more detailed
framework for mobile security, see the OWASP_Mobile_Application_Security
Project.
©Copyright
